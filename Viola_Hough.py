...
def sobel(image):
    y_deriv_image = convolve(image, np.array(
        [[-1, -2, -1], [0, 0, 0], [1, 2, 1]]))
    x_deriv_image = convolve(image, np.array(
        [[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]]))
    magnitude_image, direction_image = cv.cartToPolar(
        x_deriv_image, y_deriv_image)
...
def hough(image_name, rmin, rmax, rinc, ainc, t1, t2, t3):
    ...
    for y in range(height):
        for x in range(width):
            ...
            amin = int(
                np.clip(np.floor(np.degrees(direction_image[y, x])-6), 0, 360))
            amax = int(
                np.clip(np.floor(np.degrees(direction_image[y, x])+6+1), 0, 360))
            if ((amin > 10 and amax < 80) or (amin > 100 and amax < 170) or (amin > 190 and amax < 260) or (amin > 280 and amax < 350)):
                for a in range(amin, amax, ainc):
                    if magnitude_image[y, x] > t1:
                        d = int(np.floor((x-width//2)*np.cos(np.radians(a)
                                                             )+(y-height//2)*np.sin(np.radians(a))))
                        line_space[d, a] += 1
            ...
            for d1 in range(len(dn)-1):
                for d2 in range(d1+1, len(dn)):
                    if (line_space[dn[d1], an[a1]] == 255 and line_space[dn[d2], an[a2]] == 255 and abs(an[a1] - an[a2]) > 5):
                        a = np.array([[np.cos(np.radians(an[a1])), np.sin(np.radians(an[a1]))], [
                                     np.cos(np.radians(an[a2])), np.sin(np.radians(an[a2]))]])
                        d = np.array([[dn[d1]], [dn[d2]]])
                        if (np.linalg.det(a) != 0):
                            x, y = np.linalg.solve(a, d)
                            x, y = int(np.floor(x+width//2)
                                       ), int(np.floor(y+height//2))
...
def draw(image, boxes, colour, width):
    for i in range(len(boxes)):
        cv.rectangle(image, (boxes[i, 0], boxes[i, 1]),
                     (boxes[i, 2], boxes[i, 3]), colour, width)
...
def get_objects(image_number):
    ...
    img10_darts = np.array(
        [[93, 106, 92, 106], [586, 129, 53, 82], [917, 150, 33, 63]])
...
def viola_jones(image_name, nmin):
    ...
    det = cv.CascadeClassifier(
        "darts.xml").detectMultiScale(image_grey, 1.1, nmin)
...
def viola_hough(image_name, rmin, rmax, rinc, ainc, nmin, t1, t2, t3, t4, t5, t6):
    ...
    image_number = int(image_name[5:-4])
    height, width = image.shape[:2]
    det_circles, det_lines = hough(
        image_name, rmin, rmax, rinc, ainc, t1, t2, t3)
